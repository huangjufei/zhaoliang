...
import cn.stylefeng.guns.modular.system.vo.UserVo;
...
public class ChannelUserController extends BaseController {
    ...
    private static String PREFIX = "/modular/core/channel_user/";
    ...
    public ModelAndView index(ModelAndView mav) {
    	mav.setViewName(PREFIX + "user.html");
    	mav.addObject("company", ShiroKit.getUser().getCompany());
    ...
    public ModelAndView addView(ModelAndView mav) {
    	mav.setViewName(PREFIX + "user_add.html");
    	mav.addObject("admin", ShiroKit.isAdmin());
    	mav.addObject("companyList", jpaChannelMasterRepository.findAll());
        return mav;
    }
    ...
    public String toDetailPage(String userId) {
    	return PREFIX + "detail.html";
    }
    
    @ResponseBody
    @RequestMapping("/searchOne")
    public ResponseData searchOne(Long userId) {
    	User user = jpaUserRepository.findOneByUserId(userId);
    	User createUserInfo = jpaUserRepository.findOneByUserId(user.getCreateUser());
    	UserVo userVo = new UserVo(user);
    	userVo.setCreateUserName(createUserInfo == null ? null : createUserInfo.getName());
    ...
    public ModelAndView userEdit(@RequestParam Long userId ,String company ,ModelAndView mav) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        User user = this.userService.getById(userId);
        LogObjectHolder.me().set(user);
        mav.setViewName(PREFIX + "user_edit.html");
    	mav.addObject("admin", ShiroKit.isAdmin());
    	mav.addObject("companyList", jpaChannelMasterRepository.findAll());
        return mav;
    }
    ...
    public Object list(@RequestParam(required = false) String name,
        ...
        if(ShiroKit.isAdmin()) {//管理员
        	List<User> userList = jpaUserRepository.findAllByByConditonsAndCompany(name == null ? null : "%" + name + "%"
	    			, "".equals(company) ? null : company ,"".equals(beginTime) ? null : beginTime ,"".equals(endTime) ? null : endTime);
	    	Page<UserVo> page = new Page<UserVo>();
	    	page.setRecords(UserVo.create(userList));
	    	...
	    	List<User> userList = jpaUserRepository.findAllByByConditonsAndCompany(allRoleList 
	    			, name == null ? null : "%" + name + "%"
	    			, currentCompany ,"".equals(beginTime) ? null : beginTime ,"".equals(endTime) ? null : endTime ,userId);
	    	Page<UserVo> page = new Page<UserVo>();
	    	page.setRecords(UserVo.create(userList));
	    	page.setTotal(userList.size());
	    	return LayuiPageFactory.createPageInfo(page);
    ...
    public ResponseData add(@Valid UserDto user, BindingResult result) {
        ...
        if(user.getCompany() == null) {
        	user.setCompany(ShiroKit.getUser().getCompany());
        }
    ...
    public ResponseData setRole(@RequestParam("userId") Long userId, @RequestParam("roleIds") String roleIds) {
        ...
        //删除当前用户之前所有的角色信息
        jpaUserRoleRepository.deleteAllByUserId(userId);
        if(roleIds.indexOf(",") != -1) {
	        List<UserRole> list = new ArrayList<UserRole>();
	        String[] roleIdArray = roleIds.split(",");
	        for(int i = 0 ,len = roleIdArray.length ;i < len ;++i) {
	        	list.add(new UserRole(userId ,Long.parseLong(roleIdArray[i])));
	        }
	        jpaUserRoleRepository.saveAll(list);
        }
        else {
        	jpaUserRoleRepository.saveAndFlush(new UserRole(userId, Long.parseLong(roleIds)));
        }
        return SUCCESS_TIP;
    }
    ...
    @ResponseBody
    @RequestMapping("/search/all/user")
    public Object searchAllByCompany(@RequestParam(required=true)String company) {
    	List<User> userList = jpaUserRepository.findAllByCompany(company);
